...
export function useFloraThumbnails(limit = 40) {
  const [items, setItems] = useState<FloraThumbnail[]>(() =>
    floraImages.map((url) => ({ url: cldImage(url, "icon") }))
  );
    ...
    listFloras({ limit })
      .then((floras) => {
        ...
        const withThumb = floras
          ...
          .map((f) => ({
            url: cldImage(f.thumbnailUrl, "icon"),
            id: f.shortId ?? f._id,
            ...readerPreviewFields(f),
          }));
        const fallback = floraImages.map((url) => ({ url: cldImage(url, "icon") }));
        const combined =
      ...
      .catch(() => {
        if (!cancelled)
          setItems(floraImages.map((url) => ({ url: cldImage(url, "icon") })));
      })
      .finally(() => {
