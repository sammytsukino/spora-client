...
const roleStyles: Record<UserRole, string> = {
  user: "border-black bg-white",
  creator: "border-lime-600 bg-lime-100",
  moderator: "border-sky-600 bg-sky-100",
  admin: "border-amber-600 bg-amber-100",
};

const statusStyles: Record<UserStatus, string> = {
  active: "border-lime-600 bg-lime-100",
...
export default function AdminUserManagement({
  ...
  return (
    <section className="border-2 border-black bg-[#E9E9E9] p-6 overflow-x-auto">
      <div className="flex items-center justify-between gap-4 mb-4">
        ...
        {onExportUsers && (
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 border-2 border-black hover:bg-black hover:text-lime-300 font-jetbrains-mono text-[10px] uppercase"
          >
      ...
      <div className="border-2 border-black bg-white min-w-[720px]">
        <table className="w-full font-jetbrains-mono text-[11px]">
          <thead>
            <tr className="border-b-2 border-black bg-[#E9E9E9]">
          ...
          <tbody>
            {users.map((user) => (
              <tr
                key={user.id}
                className="border-b-2 border-black hover:bg-lime-300 transition-colors align-top"
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {onUserClick && (
                      <button
                        type="button"
                        onClick={() => onUserClick(user)}
                        className="inline-flex items-center gap-1 px-2 py-1 border-2 border-black hover:bg-black hover:text-lime-300 text-[10px] uppercase"
                      >
                    ...
                    {onActivate && (user.status === "suspended" || user.status === "banned") && (
                      <button
                        ...
                        className="inline-flex items-center gap-1 px-2 py-1 border-2 border-lime-600 text-lime-700 hover:bg-lime-600 hover:text-white text-[10px] uppercase"
                      >
                          ...
                          (role) =>
                            role !== user.role && (
                              <button
                                ...
                                className="px-2 py-0.5 border-2 border-black hover:bg-black hover:text-lime-300 text-[10px] uppercase"
                              >
                                {role}
                              </button>
                            )
