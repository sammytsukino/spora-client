...
export default function AdminMetrics({ metrics, onExportMetrics }: AdminMetricsProps) {
  return (
    <section
      className="border-2 border-black bg-[#E9E9E9] px-6 py-4 flex flex-wrap items-center justify-between gap-4"
      aria-label="Admin metrics"
    >
      <div className="flex flex-wrap items-center gap-x-8 gap-y-2 font-jetbrains-mono">
        {metricKeys.map((key) => (
      ...
      {onExportMetrics && (
        <button
          ...
          className="inline-flex items-center gap-1.5 px-3 py-1.5 border-2 border-black hover:bg-black hover:text-lime-300 font-jetbrains-mono text-[10px] uppercase shrink-0"
        >
