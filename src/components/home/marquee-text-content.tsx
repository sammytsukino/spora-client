  ...
  return (
    ...
    >
      {textItems.map((item, index) => (
        ...
        >
          <p className="font-supply-mono text-xs md:text-sm lg:text-sm text-(--spora-primary) mb-2">
            ({item.number}) {item.title}
          </p>
          <p className="font-bizud-mincho-bold text-3xl md:text-4xl lg:text-5xl text-(--spora-primary) leading-tight hover:underline">
            {item.description}
          </p>
        </button>
      ))}
